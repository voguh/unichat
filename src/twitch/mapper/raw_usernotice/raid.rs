...
pub fn parse(channel: String, tags: &HashMap<String, Option<String>>) -> Result<Option<UniChatEvent>, Error> {
    ...
    let author_profile_picture_url = tags.get("msg-param-profileImageURL").and_then(|v| v.as_ref()).ok_or(anyhow::anyhow!("Missing msg-param-profileImageURL tag"))?;
    ...
    let count_str = tags.get("msg-param-viewerCount").and_then(|v| v.as_ref()).ok_or(anyhow::anyhow!("Missing msg-param-viewerCount tag"))?;

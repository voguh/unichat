...
impl IRCMessage {
    pub fn parse(value: Option<&serde_json::Value>) -> Result<Self, Error> {
        if let Some(value) = value {
            let raw = value.get("raw").and_then(|r| r.as_str()).ok_or(anyhow::anyhow!("Missing raw IRC message"))?;
            ...
            return Err(anyhow::anyhow!("Missing IRC message value"));
        }
    }
    ...
    fn parse_prefix(prefix_arr: Option<&serde_json::Value>) -> Result<Option<IRCPrefix>, Error> {
        if let Some(prefix_arr) = prefix_arr.and_then(|prefix| prefix.as_array()) {
            if prefix_arr.len() == 0 {
                ...
                let server_name = prefix_arr[0].as_str().ok_or(anyhow::anyhow!("Invalid server name in prefix"))?;
                return Ok(Some(IRCPrefix::Server(server_name.to_string())));
            } else if prefix_arr.len() == 3 {
                let nick = prefix_arr[0].as_str().ok_or(anyhow::anyhow!("Invalid nickname in prefix"))?;
                let user = prefix_arr[1].as_str().ok_or(anyhow::anyhow!("Invalid user in prefix"))?;
                let host = prefix_arr[2].as_str().ok_or(anyhow::anyhow!("Invalid host in prefix"))?;
                return Ok(Some(IRCPrefix::Nick(nick.to_string(), user.to_string(), host.to_string())));
            }
        }

        return Err(anyhow::anyhow!("Invalid prefix array length"));
    }
    ...
    fn parse_command(command_obj: Option<&serde_json::Value>) -> Result<IRCCommand, Error> {
        if let Some(command_obj) = command_obj.and_then(|cmd| cmd.as_object()) {
            let name = command_obj.get("name").and_then(|n| n.as_str()).ok_or(anyhow::anyhow!("Invalid command name"))?;
            let params = command_obj.get("params").and_then(|params| params.as_array()).ok_or(anyhow::anyhow!("Invalid command params"))?;
        ...
        return Err(anyhow::anyhow!("Invalid command object"));
    }
}

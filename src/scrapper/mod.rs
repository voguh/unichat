...
pub fn get_scrappers() -> Result<Vec<Arc<dyn UniChatScrapper + Send + Sync>>, Error> {
    let scrappers = SCRAPPERS.read().map_err(|_| anyhow!("{} lock poisoned", LAZY_LOCK_NAME))?;
...
fn handle_event(payload: &str) -> Result<(), Error> {
    ...
    let scrappers = SCRAPPERS.read().map_err(|_| anyhow!("{} lock poisoned", LAZY_LOCK_NAME))?;
    let scrapper = scrappers.get(scrapper_id).ok_or(anyhow!("Scrapper with ID '{}' not found", scrapper_id))?;
...
pub fn register_scrapper(app: &tauri::AppHandle<tauri::Wry>, scrapper: Arc<dyn UniChatScrapper + Send + Sync>) -> Result<WebviewWindow, Error> {
    if !SCRAPPER_ID_REGEX.is_match(scrapper.id()) {
        return Err(anyhow!("Scrapper ID must be a non-empty lowercase string".to_string()));
    }

    /* ========================================================================================== */

    let mut scrappers = SCRAPPERS.write().map_err(|_| anyhow!("{} lock poisoned", LAZY_LOCK_NAME))?;
    if scrappers.contains_key(scrapper.id()) {
        return Err(anyhow!("Scrapper with ID '{}' is already registered", scrapper.id()));
    }
